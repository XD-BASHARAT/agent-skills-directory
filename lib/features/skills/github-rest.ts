...
export async function fetchOwnerInfo(owner: string): Promise<OrgInfo | null> {
  try {
    const response = await fetch(
    ...
    if (data.type === "Organization") {
      ...
      if (orgResponse.ok) {
        ...
        return {
          isVerified: orgData.is_verified === true,
          type: "Organization",
        }
      }
    }
...
export async function batchFetchOwnersVerification(
  owners: string[]
  ...
  for (let i = 0; i < uniqueOwners.length; i += BATCH_SIZE) {
    ...
    await Promise.all(
      batch.map(async (owner) => {
        const info = await fetchOwnerInfo(owner)
        results.set(owner, info?.isVerified ?? false)
      })
    )
    
    if (i + BATCH_SIZE < uniqueOwners.length) {
